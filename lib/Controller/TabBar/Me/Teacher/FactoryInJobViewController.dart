 ...
 * 审核入职事件
 */
class FactoryInJobViewController extends StatefulWidget {

  String title;
  Teacher teacher;
  FactoryInJobViewController({this.title, this.teacher});

  @override
  _FactoryInJobViewControllerState createState() => _FactoryInJobViewControllerState();
}

class _FactoryInJobViewControllerState extends State<FactoryInJobViewController> {

  List events = [];
  RefreshController _refreshController;

  _onRefresh() async {
    events = await FactoryManager.auditPATeacher(fid: widget.teacher.id);
        ...
        builder: (i, c) {
          return SmartRefresher(
              ...
              child: events.length == 0 ? Error404View(text: "0个职位报名",) : ListView.builder(
                itemBuilder: (_, index) {
                  EventTeacherAudit event = events[index];
                  ...
                  if (event.logs?.length > 0) {
                    SBLogs lastLog = event.logs.last;
                        ...
                        children: <Widget>[
                          InkWell(
                              child: ListTile(
                                ...
                                title: Text(log.user.username +":"+ log.role.title, style: TextStyle(fontSize: 14)),
  ...
  _audit(int eid, {int status = 1}) async {
    ...
    CardViewSeriesBottomSheet(
        ...
        onCorrect: (value) async{
          ...
          Progresshud.show();
          await FactoryManager.inJobReview(eid: eid, remark: value, status: status);
          Progresshud.dismiss();
          Navigator.of(context).pop();
          _onRefresh();
        }
